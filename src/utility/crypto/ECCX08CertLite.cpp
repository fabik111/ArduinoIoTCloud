...
int ECCX08CertClass::beginCSR(int keySlot, bool newPrivateKey) {
  ...
  return WiFi.iotBeginCSR(keySlot, newPrivateKey);
}

String ECCX08CertClass::endCSR() {
  return WiFi.iotEndCSR(_subjectCommonName);
}
...
int ECCX08CertClass::beginStorage(int compressedCertSlot, int serialNumberAndAuthorityKeyIdentifierSlot) {
  ...
  return WiFi.iotBeginStorage(compressedCertSlot, serialNumberAndAuthorityKeyIdentifierSlot);
}

void ECCX08CertClass::setSignature(byte signature[]) {
  memcpy(_signature, signature, 64);
}
...
void ECCX08CertClass::setExpireYears(int expireYears) {
  _dates[4] = expireYears;
}

void ECCX08CertClass::setSerialNumber(const byte serialNumber[]) {
  memcpy(_serialNumber, serialNumber, SERIAL_NUMBER_LENGTH);
}

void ECCX08CertClass::setAuthorityKeyIdentifier(const byte authorityKeyIdentifier[]) {
  memcpy(_authorityKeyIdentifier, authorityKeyIdentifier, AUTHORITY_KEY_IDENTIFIER_LENGTH);
}

int ECCX08CertClass::endStorage() {

  return WiFi.iotEndStorage(_signature, _authorityKeyIdentifier, _serialNumber, _dates);
...
int ECCX08CertClass::beginReconstruction(int keySlot, int compressedCertSlot, int serialNumberAndAuthorityKeyIdentifierSlot) {
  ...
  return WiFi.iotBeginReconstruction(_keySlot, _compressedCertSlot, _serialNumberAndAuthorityKeyIdentifierSlot);
}

int ECCX08CertClass::endReconstruction() {
  int length = WiFi.iotEndReconstruction(_issuerCountryName, _issuerOrganizationName, _issuerOrganizationalUnitName, _issuerCommonName);
  if(length == 0){
    return 0;
  }
  _bytes = (byte*)realloc(_bytes, length);
  return 1;
}

byte* ECCX08CertClass::bytes() {
  WiFi.iotGetCert(_bytes);
  return _bytes;
}
