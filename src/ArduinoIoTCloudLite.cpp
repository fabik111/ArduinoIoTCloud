...
ArduinoIoTCloudLite::ArduinoIoTCloudLite():
...
int ArduinoIoTCloudLite::begin(char const *  SSID, char const *  PASS, String brokerAddress , uint16_t brokerPort)
{

  //time_service.begin(&connection);
  WiFi.iotBegin(SSID,PASS);
  return begin(_brokerAddress, _brokerPort);
}

int ArduinoIoTCloudLite::begin(String brokerAddress, uint16_t brokerPort)
{
  ...
  _mqttClient->onMessage(ArduinoIoTCloudLite::onMessage);*/
  WiFi.MQTTsetKeepAliveInterval(30 * 1000);
  //_mqttClient->setKeepAliveInterval(30 * 1000);
  WiFi.MQTTsetConnectionTimeout(1500);
  //_mqttClient->setConnectionTimeout(1500);
  WiFi.MQTTsetID(getDeviceId().c_str());
...
NetworkConnectionState ArduinoIoTCloudLite::checkPhyConnection()
{
  ...
  if ((NetworkConnectionState)WiFi.connectionCheck() != NetworkConnectionState::CONNECTED)
  {
    ...
    {
      ArduinoIoTCloudLite::disconnect();
    }
  }

  return connect_state;
}
void ArduinoIoTCloudLite::update()
{
  // Check if a primitive property wrapper is locally changed
  _thing.updateTimestampOnLocallyChangedProperties();

  if(ArduinoIoTCloudLite::checkPhyConnection()   != NetworkConnectionState::CONNECTED)     return;
...
int ArduinoIoTCloudLite::connected()
{
  return WiFi.MQTTconnected();
}

void ArduinoIoTCloudLite::printDebugInfo()
{
...
int ArduinoIoTCloudLite::reconnect()
{
...
int ArduinoIoTCloudLite::connect()
{
  if (!WiFi.MQTTconnect(_brokerAddress.c_str(), _brokerPort)) return CONNECT_FAILURE;
  if (WiFi.MQTTsubscribe(_stdinTopic) == 0)                   return CONNECT_FAILURE_SUBSCRIBE;
  if (WiFi.MQTTsubscribe(_dataTopicIn) == 0)                  return CONNECT_FAILURE_SUBSCRIBE;

  if (_shadowTopicIn != "")
  {
    if (WiFi.MQTTsubscribe(_shadowTopicIn) == 0)              return CONNECT_FAILURE_SUBSCRIBE;
...
void ArduinoIoTCloudLite::disconnect()
{
  WiFi.MQTTstop();
}
...
void ArduinoIoTCloudLite::onMessage(int length)
{
  ArduinoCloud.handleMessage(length);
}

void ArduinoIoTCloudLite::handleMessage(int length)
{
  String topic = WiFi.MQTTmessageTopic();

  byte bytes[length];

  for (int i = 0; i < length; i++) {
    bytes[i] = WiFi.MQTTread();
  }
...
void ArduinoIoTCloudLite::sendPropertiesToCloud()
{
...
void ArduinoIoTCloudLite::requestLastValue()
{
...
ArduinoIoTConnectionStatus ArduinoIoTCloudLite::checkCloudConnection()
{
...
int ArduinoIoTCloudLite::write(String const topic, byte const data[], int const length)
{
  if (WiFi.MQTTbeginMessage(topic, length, false, 0)) {
    if (WiFi.MQTTwrite(data, length)) {
      if (WiFi.MQTTendMessage()) {
        return 1;
      }
    }
  }
...
ArduinoIoTCloudLite ArduinoCloud;
